...
pmd_t *mm_find_pmd(struct mm_struct *mm, unsigned long address)
{
	...
	pmde = *pmd;
	barrier(); 
    // pmd 가 memory 에 올라와있지 않은 상태면 0 return(PRESENT flag 가 0 이면)
    // pmd_trans_huge 는 뭐지?..
...
{
    // Anonymous page 가 아닌지 검사 
    // 아니라면 page_remove_file_rmap 수행
	if (!PageAnon(page))
    ...
    // 현재 삭제하려는 내용이 hpage 로 옮겨진 page 들을 참조한느 pte 가 여전히 있나?
	/* page still mapped by someone else? */
	if (!atomic_add_negative(-1, &page->_mapcount))
		return;
